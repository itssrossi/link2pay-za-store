-- Update the handle_new_user function to also enroll users in WhatsApp campaigns
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = NEW.id) THEN
    ...
    -- Enroll user in email drip campaigns
    PERFORM public.enroll_user_in_drip_campaigns(NEW.id);
    
    -- Enroll user in WhatsApp drip campaigns
    PERFORM public.enroll_user_in_whatsapp_campaigns(NEW.id);
  END IF;
